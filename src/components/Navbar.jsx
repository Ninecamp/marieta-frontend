  ...
  return (
    <div
      className={`fixed top-0 left-0 w-full transition-all duration-300 z-50 py-1 ${
        isScrolled ? 'bg-white shadow-md' : isSmallScreen ? 'bg-white shadow-md' : 'bg-transparent'
      }`}
    >
      <div
        className={`items-center justify-center px-8 py-4 ${
          isScrolled ? 'flex relative' : isSmallScreen ? 'flex' : 'flex flex-col'
        }`}
      >
        {isSmallScreen && (
          <button
          ...
          <img
            src={`${
              isScrolled ? '/png/M-logo-11Dark.png' : isSmallScreen? '/png/M-logo-11Dark.png': '/png/M-logo-11.png'
            }`}
            alt="Logo"
            width={`${isScrolled ? '50' : isSmallScreen ? '50' : '80'}`}
            height={`${isScrolled ? '50' : isSmallScreen ? '50' : '80'}`}
            className={`transition-transform duration-300 ${isScrolled && !isSmallScreen && "absolute left-8 mt-5"} ${!isSmallScreen && "mb-6"} `}
          />

        {!isSmallScreen && (
          <nav className={`hidden fontWeight-large md:flex gap-8 uppercase ${textStyles} ${isScrolled && "self-center"}`}>
            <Link 
              to="/" 
              className={`font-montserrat transition-colors ${getNavItemColor('home')}`}
            ...
            >
              <span
                className={`transition-colors ${getNavItemColor('menu')} flex items-center cursor-pointer`}
              ...
              {showDialog && (
                ...
                >
                  <a
                    ...
                    className="font-montserrat fontWeight-large block hover:[#d5d5d5] transition-colors"
                  >
                    Food
                  </a>
                  <a
            ...
            <Link 
              to="/contact" 
              className={`font-montserrat fontWeight-large transition-colors ${getNavItemColor('contact')}`}
      ...
      {isMenuOpen && isSmallScreen && (
        <nav className={`flex flex-col bg-white p-4 md:hidden text-left pl-8 ${textStyles}`}>
          <Link 
            to="/" 
          ...
          <div className="py-2">
            <div 
              className={`flex items-center justify-between pr-8 ${getNavItemColor('menu')}`}
            ...
            {showMobileSubmenu && (
              <div className="pl-4 mt-2 space-y-2">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block text-[#324c22] hover:text-black transition-colors"
                >
                  Food
                </a>
                <a
