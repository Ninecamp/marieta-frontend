...
const Admin = () => {
  ...
  const [currentFoodPdf, setCurrentFoodPdf] = useState({
    url: "",
    filename: "",
  });
  const [currentBarPdf, setCurrentBarPdf] = useState({ url: "", filename: "" });
  ...
  const fetchCurrentPDFs = async () => {
    try {
      const food = await getPDF1();
      setCurrentFoodPdf(food);
      const bar = await getPDF2();
      setCurrentBarPdf(bar);
    } catch (error) {
      setMessage({ text: "Failed to fetch current PDFs", type: "error" });
  ...
  const handleUpload = async (type) => {
    ...
    try {
      ...
      formData.append(type === "food" ? "foodPdf" : "barPdf", selectedFile);

      if (type === "food") {
        response = await updatePDF1(formData);
        setFoodPdf(null);
        setFoodPdfName("No file selected");
        setCurrentFoodPdf(response);
      } else {
        response = await updatePDF2(formData);
        setBarPdf(null);
        setBarPdfName("No file selected");
        setCurrentBarPdf(response);
      }
      setMessage({
        text: `${
  ...
  const handlePdfDownload = async (type) => {
    const response =
      type === "food" ? await downloadPDF1() : await downloadPDF2();
    const blob = await response.blob();
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", `${type}-menu.pdf`);
